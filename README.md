...
- M3U8 PLAYER 재생 방식 변경
  - 기존에는 M3U8 파일을 제공하는 서버에서 거부하거나(CORS) 주소가 "http" 로 시작하는 경우 브라우저 권한 문제로 재생이 불가능 했습니다. 이번 버전에서는 대부분의 M3U8 을 재생하도록 변경되었으며, 이를 위해 추가 권한을 요구할 수 있습니다. M3U8 재생 시 연결 허용 관련 팝업이 뜨는 경우 "도메인 항상 허용" 또는 "모든 도메인 항상 허용" 버튼을 클릭 후 새로고침 하세요. 본 기능은 실험 중으로 알 수 없는 오류가 발생할 수 있습니다.
