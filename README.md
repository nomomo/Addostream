...
## History
* 2019-08-09 (v 1.51.0)
  *  캠페인<br />- 본 애드온은 어그로/분탕 유저를 구분하고 적은 수고를 들여 해당 유저를 무시(차단)하는 것을 하나의 주목적으로 합니다. 이를 위해 어그로 유저 존재 시 그냥 무시하시고, 애드온의 언급은 조금 줄여주시면 감사하겠습니다.
  *  도배 관련 기능 수정<br />- 도배에 더욱 디테일하게 대응하기 위하여, [채팅 숨김을 위한 반복 입력 수]와 [유저 차단을 위한 반복 입력 수]를 분리하였습니다.<br />- [채팅 내용에 링크 포함 시에만 차단] 기능을 추가하였습니다. 이를 통해 단순 도배는 숨기기만 하고 좌표 도배의 경우에만 차단하도록 할 수 있습니다. 이 때 [좌표 - 차단한 유저의 좌표 숨기기] 기능을 사용할 경우 좌표 페이지를 깔끔하게 볼 수 있습니다.<br />- 도배로 자동 차단 시 기존 메모된 내용이 있는 경우 덮어쓰지 않고 기존 메모 내용에 덧붙입니다.
  *  (실험실) 좌표 사이트에 직접 적용 기능 추가<br />- 좌표 사이트를 직접 방문하는 경우에도 [채팅매니저로 차단한 유저의 좌표 숨기기] 와  [연속된 동일 좌표 숨기기] 기능을 사용할 수 있습니다. 현재 인사걸 좌표 사이트(insagirl-hrm.appspot.com) 에만 적용됩니다.
  *  이제 차단 로그에서 닉네임에 해당하는 메모가 존재할 경우 메모된 내용을 보여줍니다.
  *  버그 수정<br />- 기본 스킨에서 이미지 미리보기 시 View 수가 제대로 보이지 않는 문제 수정<br />- gfycat 미리보기 시 업로더 이름이 undefined 로 표시되는 문제 수정<br />버그를 제보해주신 모든 분들께 감사드립니다.
* 2019-08-07 (v 1.50.7)
