...
import CSS_addostream from 'css/adddostream.css';
import CSS_microtip from 'css/microtip.css';

import CSS_theme_default from 'css/theme_default.css';
import CSS_theme_black from 'css/theme_black.css';
import CSS_theme_black_youtube from 'css/theme_black_youtube.css';
import CSS_theme_dark from 'css/theme_dark.css';
import CSS_theme_template from 'css/theme_template.css';
const CSS_theme = {
    default:CSS_theme_default,
    black:CSS_theme_black,
    black_youtube:CSS_theme_black_youtube,
    dark:CSS_theme_dark
};
//const css_addostream = require('css/adddostream.css').toString();

export function cssLoader(type, css, document){
    if (type === "URL"){
        //
        $(document).find("head").append(`<link media="screen" rel="stylesheet" type="text/css">`).attr("href", css);
    }
    else {
        //
    }
...
export function ADD_theme(theme){
    const ADD_theme_list = ["default", "black", "black_youtube", "dark"];
    ...
    // 기본 css 는 일단 무조건 추가한다.
    if($target_document.find("#ADD_default_css").length === 0){
        $target_document.find("head").append(ADD_default_string);
    }

    // 테마 css 는 무조건 제거 후 다시 추가한다.
    $target_document.find("#ADD_theme_css").remove();
    $target_document.find("head").append(theme_css_string);

    // 기존 테마 class 제거
    $(container).removeClass("theme-"+nomo_global.theme_current);
...
export function broadcaster_theme_css(){
    ...
    // 방송 모드 종료 시
    if((nomo_global.PAGE === nomo_const.C_UCHAT || nomo_global.DEBUG) && !ADD_config.broadcaster_mode && nomo_global.$GLOBAL_IFRAME_DOCUMENT !== undefined && nomo_global.$GLOBAL_IFRAME_DOCUMENT.find("#ADD_broadcaster_mode_css").length !== 0){
    ...
    // 방송 모드 시작 시
    if((nomo_global.PAGE === nomo_const.C_UCHAT || nomo_global.DEBUG) && ADD_config.broadcaster_mode && nomo_global.$GLOBAL_IFRAME_DOCUMENT !== undefined){
        // 테마 관련 기능 종료
        ADD_theme("off");
...
export async function ADD_night_mode(options){
    ...
    var night_mode_theme = ADD_config.theme_night_mode_theme;
    if(night_mode_theme !== "black" && night_mode_theme !== "black_youtube"){
        night_mode_theme = "black";
    }
