...
(async () => {
    ...
    var GM_setting = (function($, global, document){
        ...
        const settings_full = {
            ...
            broadcaster_mode : { under_dev:true, category:"broadcast", category_name:"방송 모드", depth:1, type: "checkbox", value: false, title:"[실험실] 방송 모드", desc:"채팅창을 방송에 적합한 모드로 변경<br />Xsplit 등에서 스크린 캡쳐 후, 크로마키(기본값 blue)를 이용하여 배경색을 제거할 수 있습니다." },
            broadcaster_font_size : { under_dev:true, category:"broadcast", depth:2, type: "text", value: 1.0, valid:"number", min_value:0.1, max_value:10, title:"글씨 크기 조절(배수)", desc:"새로고침 필요" },
            broadcaster_bg_color : { under_dev:true, category:"broadcast", depth:2, type: "text", value: "blue", title:"배경 색상", desc:"예) blue, white, #fff, rgb(255, 255, 255)<br />새로고침 필요" },
    ...
    */
    ...
    async function chatElemControl($line, documentElem, iframeElems){
        ...
        if(ADD_config.broadcaster_mode){
            var nick_for_broadcaster = $nick.html().replace(":","");
            if(ADD_config.broadcaster_nick_hide){
                nick_for_broadcaster = nick_for_broadcaster.substring(0,4)+"****";
            }
        ...
        if(chatting_arrive_check && ADD_config.chat_ctr){
            ...
            $(document).arrive("u-chat > iframe", {existing: true}, async iframeElems => {
                ...
                $GLOBAL_IFRAME_DOCUMENT.one("DOMNodeInserted", "div.content", async function (){
                    ...
                    // head에 CSS 추가
                    if(ADD_config.broadcaster_mode && $GLOBAL_IFRAME_DOCUMENT.find("#ADD_broadcaster_mode_css").length === 0){
