...
(async () => {
    ...
    var GM_setting = (function($, global, document){
        ...
        const settings_full = {
            ...
            insagirl_select : { under_dev:true, category:"general", depth:2, type: "radio", value: 1, title:"[실험실] 좌표 사이트 선택", desc:"", radio: {dostream: {title: "<span style='font-size:11px;'>coord.dostream.com</span>", value:1}, insagirl: {title: "<span style='font-size:11px;'>insagirl-hrm.appspot.com</span>", value:2}} },
            ...
            // alarm_webhook : { disable:true, category:"list", depth:3, type: "checkbox", value: false, title:"온라인 시 알림(webhook)", desc:"위 목록에 등록된 스트리머가 온라인이 될 때<br />데스크톱 메시지로 알림" },
    ...
    function resetAtMidnight() {
        ...
        setTimeout(async function() {
            checkNewVersion(true);              //      <-- This is the function being called at midnight.
        ...
        if(theme_use && ADD_config.theme_font_custom !== "default"){
            var theme_font_custom_string = `
                <style id="ADD_theme_font_custom_css" type="text/css">
                    ...
                    `+(GM_page === C_UCHAT ? "body," : "")+` *:not(.glyphicon), div.line, .chatContent, span.nick {
        ...
        if(chatting_arrive_check && ADD_config.chat_ctr){
            ...
            $(document).arrive("u-chat > iframe", {existing: true}, async iframeElems => {
                ...
                $GLOBAL_IFRAME_DOCUMENT.one("DOMNodeInserted", "div.content", async function (){
                    ...
                    // 웹훅 이벤트 로드
                    // if(ADD_config.alarm_webhook){
                    //     $.ajax({
                    //         //url: "https://api.twitch.tv/kraken/users?login=lol_ambition",
                    //         url: "https://api.twitch.tv/helix/webhooks/hub",
                    //         //url: "https://api.twitch.tv/helix/users/follows?first=1&to_id=152596920",
                    //         headers: {
                    //             "hub.mode": "subscribe",
                    //             "hub.topic": "https://api.twitch.tv/helix/users/follows",
                    //             "Client-ID": ADD_CLIENT_ID_TWITCH,
                    ...
                    //     })
                    //         .done(function(data){
                    //             ADD_send_sys_msg("훅 이벤트 성공");
                    //             ADD_DEBUG("Twitch API - Request 성공", data);
                    //         })
                    //         .fail(function(error){
                    //             ADD_DEBUG("Twitch API - Request failed", error);
                    //         })
    ...
    */
    ...
    function pageChange(){
        $(window).on("hashchange", function(e){   // $(window).on("popstate hashchange", function(e){
            ...
            else if(ADD_now_playing.twip_link === ""){
                $("#ADD_twip").stop(true,true).css("opacity", "0.0").hide(300);
                if($("#history_elem").length !== 0){
                    $("#history_elem").removeClass("twip_margin");
                }
    ...
    $(document).on("click", "html, ul.stream_list", function(e){
